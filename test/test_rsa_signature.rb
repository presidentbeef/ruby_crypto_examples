...
class TestRSASignature < Minitest::Test
  def setup
  ...
  def load_file path
    load File.join(__dir__, '..', 'rsa_pkey_signatures', path)
  end

  def test_generate
    assert_output(/BEGIN RSA PRIVATE KEY.*BEGIN PUBLIC KEY/m) do
  ...
  def test_invalid_signature
    ...
    File.open('signature', 'w') do |file|
      file.write "this is garbage #{rand}"
    end
